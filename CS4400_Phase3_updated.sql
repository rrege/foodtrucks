...
reg: BEGIN
	...
	IF (LENGTH(i_password) < 8) THEN LEAVE reg; END IF;
	IF (EXISTS (SELECT username FROM `User` WHERE username = i_username)) THEN LEAVE reg; END IF;

	INSERT INTO `User` VALUES (i_username, md5(i_password), i_firstname, i_lastname);

	IF (NOT(i_email IS NULL OR i_email = '')) THEN
	...
	IF (NOT(i_balance IS NULL OR i_balance <= 0)) THEN
...
BEGIN
	...
	SELECT FoodTruck.foodTruckName as ftName, FoodTruck.stationName, (capacity - (SELECT COUNT(*) FROM FoodTruck WHERE stationName = (SELECT stationName FROM FoodTruck WHERE foodTruckName = ftName))) AS remainingCapacity, COUNT(DISTINCT username) AS staffCount, COUNT(DISTINCT foodName) as menuItemCount
    FROM FoodTruck
    INNER JOIN STATION
...
BEGIN
    ...
    INSERT mn_view_foodTruck_menu_result
...
BEGIN
	...
	SELECT Customer.stationName, Station.buildingName, GROUP_CONCAT(BuildingTag.tag SEPARATOR ',') AS tags, `description`, balance
	FROM Customer
	INNER JOIN Station
	ON Customer.stationName = Station.stationName
	INNER JOIN Building
	ON Station.buildingName = Building.buildingName
	INNER JOIN BuildingTag
...
BEGIN
	...
	SELECT FoodTruck.foodTruckName, CONCAT(`User`.firstName, ' ', `User`.lastName), GROUP_CONCAT(MenuItem.foodName SEPARATOR ',') as foodNames
	FROM Customer
	INNER JOIN FoodTruck
	ON Customer.stationName = FoodTruck.stationName
	INNER JOIN `User`
	ON FoodTruck.managerUsername = `User`.username
	INNER JOIN MenuItem
	ON FoodTruck.foodTruckName = MenuItem.foodTruckName
...
BEGIN
    ...
    IF ((SELECT balance FROM Customer INNER JOIN Orders ON Customer.username = Orders.customerUsername WHERE Orders.orderID = i_orderId)
			>= (SELECT price FROM MenuItem WHERE foodName = i_foodName AND foodTruckName = i_foodTruckName) * i_purchaseQuantity) THEN
		UPDATE Customer
		SET balance = balance - (SELECT price FROM MenuItem WHERE foodName = i_foodName AND foodTruckName = i_foodTruckName) * i_purchaseQuantity
		WHERE username = (SELECT customerUsername FROM Orders WHERE Orders.orderID = i_orderId);
...
BEGIN
	...
	SELECT Orders.`date`, Orders.orderID, SUM(purchaseQuantity * price), GROUP_CONCAT(DISTINCT MenuItem.foodName SEPARATOR ','), SUM(purchaseQuantity)
	FROM Orders
	INNER JOIN OrderDetail
	ON Orders.orderID = OrderDetail.orderID
	INNER JOIN MenuItem
	ON OrderDetail.foodTruckName = MenuItem.foodTruckName AND OrderDetail.foodName = MenuItem.foodName
